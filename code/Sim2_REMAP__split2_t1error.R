...
conf_sup.he <- matrix(nrow=K/10, ncol=reps)
...
for (n in 1:reps){
  ...
  for (h in 1:T){
    for(j in 1:S){
      y.he = rbinom(1, round(nk.he[1,j,h]), 0.25)
      Y.he[1,j,h]         = y.he
...
for(j in 1:S){
  ...
  y.he = rbinom(1, round(nk.he[i,j,h]), 0.25)
...
if (i %% 10 == 0){
  ...
  pat_df <- data.frame(t=rep(1, sum(nk.he[1:i,,])), o=rep(0, sum(nk.he[1:i,,])))
  pat_df[1:sum(nk.he[1:i,1,]), 't'] = 0
  pat_df[1:(sum(Y.he[1:i,1,])), 'o'] = 1
  pat_df[(sum(nk.he[1:i,1,])+1):(sum(nk.he[1:i,1,])+sum(Y.he[1:i,2,])), 'o'] = 1
  
  
  fit = glm(o~t,data=pat_df, family=binomial)
  conf_sup.he[i/10,n] = summary(fit)$coef['t','Pr(>|z|)']
...
t1e_metric.he = vector(length=3)
for (i in 1:10){
  t1e_metric.he[i] = mean(conf_sup.he[i,]<0.05)
}

t1e_metric.he


lines(t1e_metric.he*100, lwd=4, type='b', col='red')

#legend('topright',legend=c(expression('T'[s]), expression('RMC'[s])), col=c('blue','red'), lwd=4)

legend('topright',legend=c(expression('T'[s]), expression('RMC'[s]), 'FeR','FuR'), 
