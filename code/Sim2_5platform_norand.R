...
for(j in 1:Si){
  nk.k[1, j] = rbinom(1,nk_.k, rand_ratio.k[j])
for (h in 1:T){
  y.k[h] = rbinom(1, rbinom(1,nk.k[1,j], Px.k[h]), Py.k[h,j])
}
Y.k[1,j]         = sum(y.k)
...
for(j in 1:Si){
  nk.k[i, j] = rbinom(1,nk_.k, rand_ratio.k[j])
for (h in 1:T){
  y.k[h] = rbinom(1, rbinom(1,nk.k[i,j], Px.k[h]), Py.k[h,j])
}
Y.k[i,j]         = sum(y.k)
