...
for(j in 1:Si){
  nk.i[1, j] = rbinom(1,nk_.i, rand_ratio.i[j])
for (h in 1:T){
  y.i[h] = rbinom(1, rbinom(1,nk.i[1,j], Px.i[h]), Py.i[h,j])
}
Y.i[1,j]         = sum(y.i)
...
for(j in 1:Si){
  nk.i[i, j] = rbinom(1,nk_.i, rand_ratio.i[j])
for (h in 1:T){
  y.i[h] = rbinom(1, rbinom(1,nk.i[i,j], Px.i[h]), Py.i[h,j])
}
Y.i[i,j]         = sum(y.i)
...
plot(0.875+(j-1), mid[j],
     col='blue', xaxt='n',xlim = c(0.5,3.5), ylim=c(-0.6,0.6), #ylim=c(-1.5, 1.5),
