...
for(j in 1:Si){
  nk.m[1, j] = rbinom(1,nk_.m, rand_ratio.m[j])
for (h in 1:T){
  y.m[h] = rbinom(1, rbinom(1,nk.m[1,j], Px.m[h]), Py.m[h,j])
}
Y.m[1,j]         = sum(y.m)
...
for(j in 1:Si){
  nk.m[i, j] = rbinom(1,nk_.m, rand_ratio.m[j])
for (h in 1:T){
  y.m[h] = rbinom(1, rbinom(1,nk.m[i,j], Px.m[h]), Py.m[h,j])
}
Y.m[i,j]         = sum(y.m)
