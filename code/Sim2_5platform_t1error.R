...
Py.je = matrix(rep(0.25, 12), nrow=3)
...
t_metric.je <- array(dim = c(Si-1,K/10, reps))
...
for(j in 1:Si){
  nk.je[1,j] = rbinom(1,nk_.je, rand_ratio.je[j])
#for (h in 1:T){
#  y.je[h] = rbinom(1, rbinom(1,nk.je[1,j], Px.je[h]), Py.je[h,j])
#}
Y.je[1,j]         = rbinom(1, round(nk_.je * rand_ratio.je[j]), 0.25) #sum(y.je)
...
for(j in 1:Si){
  nk.je[i, j] = rbinom(1,nk_.je, rand_ratio.je[j])
#for (h in 1:T){
#  y.je[h] = rbinom(1, rbinom(1,nk.je[i,j], Px.je[h]), Py.je[h,j])
#}
Y.je[i,j]         = rbinom(1, nk.je[i,j], 0.25)  # sum(y.je)
...
if (i %% 10 == 0){
  ...
  pat_df[(sum(nk.je[1:i,1])+1):(sum(nk.je[1:i,1])+sum(Y.je[1:i,2])), 'o'] = 1
  ...
  t_metric.je[1,i/10,n] = t.test(o~t, data = pat_df)$p.value
  ...
  pat_df[1:sum(nk.je[1:i,1]), 't'] = 0
  pat_df[1:(sum(Y.je[1:i,1])), 'o'] = 1
  pat_df[(sum(nk.je[1:i,1])+1):(sum(nk.je[1:i,1])+sum(Y.je[1:i,3])), 'o'] = 1
  
  #fit = glm(o~t,data=pat_df, family=binomial)
  #conf_sup.je[2,i/10,n] = summary(fit)$coef['t','Pr(>|z|)']
  #conf_sup_dir.je[2,i/10,n] = summary(fit)$coef['t','Estimate']
  t_metric.je[2,i/10,n] = t.test(o~t, data = pat_df)$p.value
  ...
  pat_df[1:sum(nk.je[1:i,1]), 't'] = 0
  pat_df[1:(sum(Y.je[1:i,1])), 'o'] = 1
  pat_df[(sum(nk.je[1:i,1])+1):(sum(nk.je[1:i,1])+sum(Y.je[1:i,4])), 'o'] = 1
  
  #fit = glm(o~t,data=pat_df, family=binomial)
  #conf_sup.je[3,i/10,n] = summary(fit)$coef['t','Pr(>|z|)']
  #conf_sup_dir.je[3,i/10,n] = summary(fit)$coef['t','Estimate']
  t_metric.je[3,i/10,n] = t.test(o~t, data = pat_df)$p.value
...
t1e_metric.je = vector(length=10)
for (i in 1:10){
  t1e_metric.je[i] = mean(apply(t_metric.je[,i,]<(0.05/3), 2, any))
}

t1e_metric.je

plot(t1e_metric.je*100, lwd=4, type='b', col='blue', ylim=c(0,15),
...
postmean_avg.je = matrix(nrow=K, ncol=Si)
...
for (j in 1:Si){
  ...
  postmean_avg.je[,j] = apply(postmean_a.je[,j,],1, mean)
...
type_1_err.je = vector(length=10)
type_1_err_adj.je = vector(length=10)

t1e_treats = matrix(nrow=10, ncol=3)

for (i in 1:10){
  type_1_err.je[i] = mean(apply(conf_sup.je[,i,]<=0.05, 2, any)) #0.186
  type_1_err_adj.je[i] =mean(apply(conf_sup.je[,i,]<=(0.05/3), 2, any)) #0.077
  t1e_treats[i,] = apply(conf_sup.je[,i,]<=0.05, 1, mean)
}
