...
conf_sup.ge <- array(dim=c(K/10, T,reps))
conf_sup_dir.ge <- array(dim=c(K/10, T,reps))
...
for (h in 1:T){
for(j in 1:S){
y.ge = rbinom(1, round(nk.ge[1,j,h]), Py.ge[h,j])
Y.ge[1,j,h]         = y.ge
...
for(j in 1:S){
  y.ge = rbinom(1, round(nk.ge[i,j,h]), Py.ge[h,j])
...
if (i %% 10 == 0){
  pat_df <- data.frame(t=rep(1, i*nk_.ge[h]), o=rep(0, i*nk_.ge[h]))
  pat_df[1:sum(nk.ge[1:i,1,h]), 't'] = 0
  pat_df[1:(sum(Y.ge[1:i,1,h])), 'o'] = 1
  pat_df[(sum(nk.ge[1:i,1,h])+1):(sum(nk.ge[1:i,1,h])+sum(Y.ge[1:i,2,h])), 'o'] = 1
  
  fit = glm(o~t,data=pat_df, family=binomial)
  conf_sup.ge[i/10,h,n] = summary(fit)$coef['t','Pr(>|z|)']
  conf_sup_dir.ge[i/10,h,n] = summary(fit)$coef['t','Estimate']
}
}
