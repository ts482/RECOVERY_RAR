...
for(j in 1:Si){
for (h in 1:T){
  y.me[h] = rbinom(1, round(nk.me[1,j]* Px.me[h]), Py.me[h,j])
}
Y.me[1,j]         = sum(y.me)
...
for(j in 1:Si){
  nk.me[i, j] = round(nk_.me * rand_ratio.me[j])
for (h in 1:T){
  y.me[h] = rbinom(1, round(nk.me[i,j]* Px.me[h]), Py.me[h,j])
}
Y.me[i,j]         = sum(y.me)
...
mean(apply(conf_sup.me[,8,]<0.05, 2, any))  #0.173
mean(apply(conf_sup.me[,8,]<(0.05/3), 2, any))  #0.075  #bonferroni correction
